...
async function renameClip(clip, settings) {
  ...
  const name = cleanText(settings.name, "Video Clip");
  const ok = await clip.SetName(name);

  if (!ok) {
    throw new Error("Failed to rename video clip.");
  }
}
...
async function rotationClip(clip, settings) {
  await setProperty(clip, ["RotationAngle", "Rotation"], settings.rotationAngle, 0);
}
...
async function duplicateClip(project, timeline, clip, settings) {
  ...
  const clipInfo = await buildDuplicateClipInfo(clip, duplicateToTrackIndex, timelineStart);
...
async function applyLutClip(clip, settings) {
  if (!clip || typeof clip.SetLUT !== "function") {
    throw new Error("Resolve API does not support SetLUT.");
  }

  const nodeIndex = Math.max(1, cleanNumber(settings.nodeIndex, 1));
  const lutPath = String(settings.lutPath || "").trim();

  if (!lutPath) {
    throw new Error("No LUT path specified.");
  }

  const ok = await clip.SetLUT(nodeIndex, lutPath);

  if (!ok) {
    throw new Error(`Failed to apply LUT to node ${nodeIndex}.`);
  }
}
...
async function splitClip(project, timeline, clip, settings) {
  ...
  if (playheadFrame <= timelineStart || playheadFrame >= timelineEnd) {
    throw new Error("Playhead must be inside the video clip, not on the edge.");
  }
...
async function runVideoClipTargetModule({ project, timeline, module }) {
  ...
  if (action === "lut") return applyLutClip(clip, settings);
